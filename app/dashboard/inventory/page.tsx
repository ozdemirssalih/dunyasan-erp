...
export default function InventoryPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const whItems: UnifiedItem[] = (whData || []).map((item: any) => ({
        ...
        code: item.code,
        name: item.name,
        category: item.category_name || 'Depo',
      ...
      // 3. production_inventory tablosu
      ...
      const merged = [...invItems, ...whItems, ...prodItems]
      setAllItems(merged)
  ...
  const getStockStatus = (item: UnifiedItem) => {
    ...
    if (item.min_stock > 0 && item.quantity < item.min_stock)
      return { label: 'Düşük Stok', color: 'text-orange-600', bg: 'bg-orange-50' }
    ...
    return (
      <div className="flex items-center justify-center h-full min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-gray-600">Tüm stoklar yükleniyor...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">
              Tüm stoklar: Envanter · Depo · Üretim
            </p>
          </div>
          <div className="flex items-center gap-2 text-sm text-gray-500">
            <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full font-semibold">{invCount} Stok</span>
            <span className="bg-purple-100 text-purple-700 px-3 py-1 rounded-full font-semibold">{whCount} Depo</span>
            <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full font-semibold">{prodCount} Üretim</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            ...
            <p className="text-xs text-gray-400 mt-1">acil temin gerekiyor</p>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 flex flex-wrap gap-3 items-center">
          <input
            ...
            className="flex-1 min-w-[200px] px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm"
          />
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="px-6 py-3 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
            <span className="text-sm text-gray-600 font-medium">{filteredItems.length} kalem gösteriliyor</span>
          </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Stok Durumu</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredItems.length > 0 ? filteredItems.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className={`hover:bg-gray-50 transition-colors ${item.quantity === 0 ? 'opacity-60' : ''}`}>
                      <td className="px-5 py-3.5">
                        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${source.bg}`}>
                          {source.label}
                        </span>
                      </td>
                      <td className="px-5 py-3.5">
                      ...
                      <td className="px-5 py-3.5">
                        ...
                        {item.min_stock > 0 && (
                          <p className="text-xs text-gray-400 mt-0.5">Min: {item.min_stock}</p>
                        )}
                      </td>
                      <td className="px-5 py-3.5">
                        ...
                        {item.min_stock > 0 && (
                          <div className="mt-1.5 w-24 bg-gray-200 rounded-full h-1.5">
                            <div
                              className={`h-1.5 rounded-full ${
                                item.quantity === 0 ? 'bg-red-500' :
                                item.quantity < item.min_stock ? 'bg-orange-400' : 'bg-green-500'
                              }`}
                      ...
                      <td className="px-5 py-3.5 text-sm text-gray-600">
                        {item.unit_price ? `${item.unit_price.toLocaleString('tr-TR')} ₺` : '—'}
                      </td>
