...
import { FileText, Plus, X, Edit3, Trash2, Search, CheckCircle2, Clock, Eye } from 'lucide-react'
...
const STATUS_MAP: Record<string, { label: string; bg: string; text: string }> = {
  ...
  in_progress: { label: 'Üretimde', bg: 'bg-yellow-100', text: 'text-yellow-700' },
...
export default function WorkOrdersPage() {
  ...
  const handleSave = async () => {
    if (!form.parca_adi || !companyId) return alert('Parça adı zorunludur!')
    try {
      ...
      const payload = {
        ...
        project_id: form.project_id || null, project_name: projectName,
        ...
        dosya_no: form.dosya_no || null, planlanan_miktar: parseInt(form.planlanan_miktar) || 0,
        ...
        is_merkezi: form.is_merkezi || null, uygun_miktar: parseInt(form.uygun_miktar) || 0,
        ret_miktar: parseInt(form.ret_miktar) || 0, uygunsuzluk_no: form.uygunsuzluk_no || null,
  ...
  const filtered = orders.filter(o => {
    ...
    if (searchTerm) {
      ...
      return [o.parca_no, o.parca_adi, o.iem_no, o.project_name, o.customer_name, o.malzeme, o.operasyon_no]
    ...
    <PermissionGuard module="planning" permission="view">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-bold text-gray-800 flex items-center gap-2"><FileText className="w-8 h-8 text-blue-600" /> İş Emirleri</h2>
            <p className="text-gray-600">Üretim iş emirlerini oluşturun ve takip edin</p>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border p-4 flex gap-4 flex-wrap">
          <div className="flex-1 relative min-w-[200px]">
            ...
            <input type="text" placeholder="Parça no, ad, İEM no, proje, müşteri ara..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b">
                <tr>
                  <th className="px-3 py-3 text-left text-xs font-semibold text-gray-600">İEM No</th>
                  <th className="px-3 py-3 text-left text-xs font-semibold text-gray-600">Parça No</th>
                  <th className="px-3 py-3 text-left text-xs font-semibold text-gray-600">Parça Adı</th>
                  <th className="px-3 py-3 text-left text-xs font-semibold text-gray-600">Proje</th>
                  <th className="px-3 py-3 text-left text-xs font-semibold text-gray-600">Müşteri</th>
                  ...
                  <th className="px-3 py-3 text-center text-xs font-semibold text-gray-600">Doğrulama</th>
                  <th className="px-3 py-3 text-right text-xs font-semibold text-gray-600">İşlem</th>
                </tr>
              </thead>
                ...
                {filtered.map(wo => {
                  ...
                  return (
                    <tr key={wo.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-3 text-gray-600">{wo.project_name || '-'}</td>
            ...
            {filtered.length === 0 && <div className="text-center py-12 text-gray-400">İş emri bulunamadı</div>}
          </div>
        </div>
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowDetail(null)}>
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-bold text-gray-800">İş Emri Detayı — {showDetail.iem_no || showDetail.parca_adi}</h3>
                <button onClick={() => setShowDetail(null)}><X className="w-5 h-5 text-gray-400" /></button>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                {[
                  ['Parça No', showDetail.parca_no], ['Parça Adı', showDetail.parca_adi], ['Proje', showDetail.project_name],
                  ['İEM No', showDetail.iem_no], ['Revizyon No', showDetail.revizyon_no], ['FAI', showDetail.fai],
                  ['Seri', showDetail.seri], ['Delta FAI', showDetail.delta_fai], ['Müşteri', showDetail.customer_name],
                  ...
                  ['Malzeme', showDetail.malzeme], ['Alaşım & Spec', showDetail.alasim_spec], ['Operasyon No', showDetail.operasyon_no],
                  ['İş Merkezi', showDetail.is_merkezi], ['Uygun Miktar', showDetail.uygun_miktar], ['Ret Miktar', showDetail.ret_miktar],
                  ['Uygunsuzluk No', showDetail.uygunsuzluk_no], ['Ekipman', showDetail.ekipman],
                  ['Başlama', showDetail.baslama_tarihi ? new Date(showDetail.baslama_tarihi).toLocaleDateString('tr-TR') : '-'],
                  ['Bitiş', showDetail.bitis_tarihi ? new Date(showDetail.bitis_tarihi).toLocaleDateString('tr-TR') : '-'],
                  ['Doğrulama', showDetail.dogrulama ? '✅ Doğrulandı' : '⏳ Bekliyor'], ['Doğrulayan', showDetail.dogrulayan],
                ].map(([label, value], i) => (
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="sticky top-0 bg-white border-b px-6 py-4 flex justify-between items-center rounded-t-2xl z-10">
                <h3 className="text-xl font-bold text-gray-800">{editingId ? 'İş Emri Düzenle' : 'Yeni İş Emri'}</h3>
              ...
              <div className="p-6 space-y-5">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <F label="Parça No"><input type="text" value={form.parca_no} onChange={e => setForm({...form, parca_no: e.target.value})} className={inp} /></F>
                    <F label="Parça Adı *"><input type="text" value={form.parca_adi} onChange={e => setForm({...form, parca_adi: e.target.value})} className={inp} /></F>
                    <F label="İEM No"><input type="text" value={form.iem_no} onChange={e => setForm({...form, iem_no: e.target.value})} className={inp} /></F>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    <F label="Proje"><select value={form.project_id} onChange={e => setForm({...form, project_id: e.target.value})} className={inp}><option value="">Seçin...</option>{projects.map(p => <option key={p.id} value={p.id}>{p.project_name}</option>)}</select></F>
                    <F label="Müşteri"><select value={form.customer_id} onChange={e => setForm({...form, customer_id: e.target.value})} className={inp}><option value="">Seçin...</option>{customers.map(c => <option key={c.id} value={c.id}>{c.contact_name}</option>)}</select></F>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    <F label="Malzeme"><input type="text" value={form.malzeme} onChange={e => setForm({...form, malzeme: e.target.value})} placeholder="Ör: Al 7075-T7351" className={inp} /></F>
                    <F label="Alaşım & Spec"><input type="text" value={form.alasim_spec} onChange={e => setForm({...form, alasim_spec: e.target.value})} placeholder="Ör: AMS-QQ-A-250/12" className={inp} /></F>
                ...
                {/* Operasyon */}
                <div>
                  <h4 className="text-sm font-bold text-gray-700 mb-3 border-b pb-1">Operasyon & Üretim</h4>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <F label="İş Merkezi"><input type="text" value={form.is_merkezi} onChange={e => setForm({...form, is_merkezi: e.target.value})} className={inp} /></F>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <F label="Başlama Tarihi"><input type="date" value={form.baslama_tarihi} onChange={e => setForm({...form, baslama_tarihi: e.target.value})} className={inp} /></F>
                    <F label="Bitiş Tarihi"><input type="date" value={form.bitis_tarihi} onChange={e => setForm({...form, bitis_tarihi: e.target.value})} className={inp} /></F>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <F label="Doğrulayan"><input type="text" value={form.dogrulayan} onChange={e => setForm({...form, dogrulayan: e.target.value})} placeholder="İsim" className={inp} /></F>
                  </div>
                </div>
              ...
              <div className="sticky bottom-0 bg-gray-50 border-t px-6 py-4 flex gap-3 rounded-b-2xl">
                <button onClick={() => setShowModal(false)} className="flex-1 py-2.5 border border-gray-300 text-gray-700 rounded-lg font-semibold hover:bg-gray-100">İptal</button>
