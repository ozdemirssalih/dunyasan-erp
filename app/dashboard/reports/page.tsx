      ...
      <div ref={reportRef}>
        ...
        {tab === 'production' && (
          <div className="space-y-6">
            ...
            {prod.daily.length > 0 && <div className="bg-white rounded-xl shadow-sm border p-5"><h3 className="font-bold text-gray-800 mb-4">Günlük Üretim</h3><ResponsiveContainer width="100%" height={250}><BarChart data={prod.daily}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="date" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip /><Bar dataKey="Üretim" fill="#3b82f6" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer></div>}
            {prod.daily.length > 0 && <div className="bg-white rounded-xl shadow-sm border p-5"><h3 className="font-bold text-gray-800 mb-4">Günlük Fire ve Fire Oranı</h3><ResponsiveContainer width="100%" height={250}><ComposedChart data={prod.daily.map((d: any) => ({ ...d, 'Fire %': d.Üretim > 0 ? Math.round(d.Fire / d.Üretim * 1000) / 10 : 0 }))}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="date" tick={{ fontSize: 10 }} /><YAxis yAxisId="left" tick={{ fontSize: 10 }} /><YAxis yAxisId="right" orientation="right" tick={{ fontSize: 10 }} unit="%" /><Tooltip /><Legend /><Bar yAxisId="left" dataKey="Fire" fill="#ef4444" radius={[4, 4, 0, 0]} /><Line yAxisId="right" type="monotone" dataKey="Fire %" stroke="#f97316" strokeWidth={2} dot={{ r: 3 }} /></ComposedChart></ResponsiveContainer></div>}
