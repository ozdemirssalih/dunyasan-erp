      ...
      <div ref={reportRef}>
        ...
        {tab === 'production' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {prod.byProject.length > 0 && <div className="bg-white rounded-xl shadow-sm border p-5">
                ...
                <div className="mt-6 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl">
                  ...
                  <p className="text-sm text-gray-700 leading-relaxed">
                    {prod.byPart.length > 0 && (() => { const partTotal = prod.byPart.reduce((s: number, p: any) => s + p.total, 0); const topPart = prod.byPart[0]; const bottomPart = prod.byPart[prod.byPart.length - 1]; return <>Üretim takibinde <strong className="text-purple-600">{prod.byPart.length}</strong> farklı parça üretilmiş, toplam <strong className="text-purple-600">{f(partTotal)}</strong> adet çıktı kaydedilmiştir. En çok üretilen parça <strong className="text-purple-600">{topPart?.name}</strong> (<strong className="text-purple-600">{f(topPart?.total)}</strong> adet, toplam çıktının <strong className="text-purple-600">%{partTotal > 0 ? (topPart.total / partTotal * 100).toFixed(0) : '0'}</strong>{"'"}i){prod.byPart.length > 1 && <>, en az üretilen parça <strong className="text-purple-600">{bottomPart?.name}</strong> (<strong className="text-purple-600">{f(bottomPart?.total)}</strong> adet)</>} olmuştur.{' '}</> })()}
                    {prod.topWorkers.length > 0 && <>En verimli çalışan <strong className="text-green-600">{prod.topWorkers[0]?.name}</strong> (<strong className="text-green-600">%{prod.topWorkers[0]?.eff}</strong> verimlilik, {f(prod.topWorkers[0]?.total)} işlem) olmuştur.{' '}</>}
                    {prod.topProducers.length > 0 && prod.topProducers[0]?.name !== prod.topWorkers[0]?.name && <>En çok üretim yapan çalışan ise <strong className="text-blue-600">{prod.topProducers[0]?.name}</strong> (<strong className="text-blue-600">{f(prod.topProducers[0]?.total)}</strong> işlem) olmuştur.{' '}</>}
                    Seçilen tarih aralığında toplam <strong className="text-blue-600">{prod.byProject.length}</strong> projede, <strong className="text-blue-600">{prod.byMachine.length}</strong> tezgahta üretim gerçekleştirilmiştir.
                    Toplam <strong className="text-blue-600">{f(prod.total)}</strong> adet işlem yapılmış, <strong className="text-red-600">{f(prod.defects)}</strong> adet fire oluşmuştur (fire oranı <strong className="text-red-600">%{prod.total > 0 ? (prod.defects / prod.total * 100).toFixed(1) : '0'}</strong>).
                    En yüksek işlem kapasitesi <strong className="text-blue-600">{prod.byProject[0]?.name}</strong> projesine ait olup <strong className="text-blue-600">{f(prod.byProject[0]?.total)}</strong> adet işlem kaydedilmiştir.
